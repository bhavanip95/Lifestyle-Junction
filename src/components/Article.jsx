...
// import HairCare from '../assets/CategoryConfigImages/hairCare.png';
...
//   data-theme="retro"
//   role="tablist"
//   className="tabs tabs-lift tabs-xl flex justify-evenly font-semibold"
// >
...
//       className="tab"
//       aria-label={sc.title}
//       checked={selectedSub === sc.key}
//       onChange={() => setSelectedSub(sc.key)}
//     />
//   ))}
// </div>
...
// <div data-theme="caramellatte" className="hero my-8">
//         <div className="hero-content flex-col lg:flex-row">
//           <img src={cfg.heroImage} className="max-w-sm rounded-lg shadow-2xl" />
//           <div>
//             <h1 className="text-5xl font-bold">{cfg.heroTitle}</h1>
//             <p className="py-6">{cfg.heroSubtitle}</p>
//           </div>
//         </div>
//       </div>
...
// <div data-theme="dim" className="flex flex-wrap gap-2 p-4">
//         {cfg.badges?.map((tag) => (
//           <div
//             key={tag}
//             className="badge badge-outline badge-xl"
...
export default function Article() {
  ...
  // initialize selectedSub either from URL param or default to first subCategory.key
  const [selectedSub, setSelectedSub] = useState(
    subCategory || cfg.subCategories[0]?.key
  );
  const [articles, setArticles] = useState([]);

  // whenever selectedSub changes, push new URL and fetch data
  useEffect(() => {
    // update the URL if it doesn't match the state
    if (subCategory !== selectedSub) {
      navigate(`/articles/${category}/${selectedSub}`, { replace: true });
    }

    // simulate API call
    setTimeout(() => {
      const data = (dummyData[category] || []).filter(a =>
        selectedSub === 'All'
          ? true
          : a.subCategory ===
            cfg.subCategories.find(sc => sc.key === selectedSub)?.title
      );
      setArticles(data);
    }, 500);
  }, [category, selectedSub, navigate, subCategory, cfg.subCategories]);

  const subCfg =
    cfg.subCategories.find(sc => sc.key === selectedSub) || {};
    ...
    <>
      ...
      <div data-theme="caramellatte">
        <div className="hero bg-base-200 max-w-screen">
          <div className="hero-content flex-col lg:flex-row">
            <img
              src={subCfg.heroImage}
              alt={subCfg.heroTitle}
              className="max-w-sm rounded-lg shadow-2xl"
            />
            <div>
              <h1 className="text-5xl font-bold">{subCfg.heroTitle}</h1>
              <p className="py-6">{subCfg.heroSubtitle}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        {articles.map((article, idx) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <Link
                to={`/ReadMoreArticle/${category}/${selectedSub}`}
